...
module codeword_generator #(
    ...
    input logic input_last_subblock_i,                     // Frame boundary delimiter for active codeword

    // Upstream Encoder Core Interface
    input logic info_valid_i,                              // Systematic data block valid strobe
    input logic [3:0][(ZC_MAX >> 2)-1:0] info_data_i,       // 384-bit packed systematic input data bus

    input logic core_parity_valid_i,                        // Core parity data block valid strobe
    input logic [3:0][(ZC_MAX >> 2)-1:0] core_parity_data_i, // 384-bit packed core parity input data bus

    input logic add_parity_valid_i,                        // Additional parity data block valid strobe
    input logic [3:0][COL_WIDTH-1:0] add_parity_idx_i,     // Array of 4 out-of-order matrix row indices
    input logic [3:0][(ZC_MAX >> 2)-1:0] add_parity_data_i, // 384-bit split additional parity data bus
 
    output logic upstream_ready_o,                         // Flow control backpressure status signal

    // Inter-Module Interface to Output Buffer
    output logic codeword_valid_o,                         // Active read bank ready status notification flag
    input  logic [COL_WIDTH-1:0] r_addr_i,                  // Subblock row address driven by output buffer
    output logic [ZC_MAX-1:0] r_data_o,                    // Recombined 384-bit row data output bus
    input  logic codeword_done_i                           // Codeword transmission finish acknowledgement strobe
);
    ...
    logic [6:0] w_seq_addr_q, w_seq_addr_n;

    // Structural Addressing and Muxing Vectors
    logic [3:0][7:0] w_addr;
    logic [3:0][(ZC_MAX >> 2)-1:0] w_data_mux;
    logic [3:0] ram_w_en;                                  // Independent write enables per sub-bank
    logic       input_stroke;

    // Phase Boundary Alignment Evaluation Wires
    logic [6:0] core_parity_base_row;
    logic [6:0] add_parity_base_row;
    logic [2:0] packing_factor;
    logic [6:0] core_parity_start_idx;
    logic [6:0] active_w_seq_addr;
    ...
    //--------------------------------------------------------------------------
    // Concern 1: Lookahead Phase Base Row and Stride Calculations
    //--------------------------------------------------------------------------
    always_comb begin
    ...
    //--------------------------------------------------------------------------
    // Concern 2: Sequential Ingestion Address Pointer Evolution
    //--------------------------------------------------------------------------
    always_comb begin
    ...
    //--------------------------------------------------------------------------
    // Concern 3: Address Steering Matrix & Sub-Bank Write Enable Decoding
    //--------------------------------------------------------------------------
    always_comb begin
        ram_w_en   = 4'b0000;
        ...
        if (info_valid_i) begin
            ...
            unique case (zc_group_i)
                ZC_SMALL: begin
                    ram_w_en[active_w_seq_addr[1:0]] = upstream_ready_o;
                    for (int unsigned i = 0; i < 4; i++) begin
                        w_addr[i] = {w_swap_q, 2'b00, active_w_seq_addr[6:2]};
                    end
                end
                ZC_MEDIUM: begin
                    if (active_w_seq_addr[0] == 1'b0) ram_w_en[1:0] = {2{upstream_ready_o}};
                    else                              ram_w_en[3:2] = {2{upstream_ready_o}};
                    for (int unsigned i = 0; i < 4; i++) begin
                        w_addr[i] = {w_swap_q, 1'b0, active_w_seq_addr[6:1]};
                    end
                end
                ZC_LARGE: begin
                    ram_w_en = {4{upstream_ready_o}};
                    for (int unsigned i = 0; i < 4; i++) begin
                        w_addr[i] = {w_swap_q, active_w_seq_addr};
                    end
                end
        ...
        // Phase B: Core Parity Ingestion (Row Aligned, Groups of 4/2/1)
            ...
            ram_w_en   = {4{upstream_ready_o}};
            unique case (zc_group_i)
                ZC_SMALL:  for (int unsigned i = 0; i < 4; i++) w_addr[i] = {w_swap_q, 2'b00, active_w_seq_addr[6:2]};
                ZC_MEDIUM: for (int unsigned i = 0; i < 4; i++) w_addr[i] = {w_swap_q, 1'b0,  active_w_seq_addr[6:1]};
        ...
        // Phase C: Additional Parity Ingestion (Out-of-order, appended past Parity blocks)
            ...
            unique case (zc_group_i)
                ZC_SMALL: begin
                    for (int unsigned i = 0; i < 4; i++) begin
                        w_addr[i] = {w_swap_q, (add_parity_base_row + add_parity_idx_i[i])};
                    end
                end
                ZC_MEDIUM: begin
                    w_addr[0] = {w_swap_q, (add_parity_base_row + add_parity_idx_i[0])};
                    w_addr[1] = {w_swap_q, (add_parity_base_row + add_parity_idx_i[0])};
                    w_addr[2] = {w_swap_q, (add_parity_base_row + add_parity_idx_i[1])};
                    w_addr[3] = {w_swap_q, (add_parity_base_row + add_parity_idx_i[1])};
                end
                ZC_LARGE: begin
                    for (int unsigned i = 0; i < 4; i++) begin
                        w_addr[i] = {w_swap_q, (add_parity_base_row + add_parity_idx_i[0])};
                    end
                end
            endcase
        end
    ...
    //--------------------------------------------------------------------------
    // Concern 4: Sub-Bank Physical Hardware Memory Writing
    //--------------------------------------------------------------------------
    ...
    //--------------------------------------------------------------------------
    // Concern 5: Read Port Structural Mapping
    //--------------------------------------------------------------------------
    ...
    //--------------------------------------------------------------------------
    // Concern 6: Control Boundaries and Swap State Machinery
    //--------------------------------------------------------------------------
