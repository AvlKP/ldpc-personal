...
logic [COL_WIDTH-1:0] w_bank_valid_addr [0:3];
logic [3:0] w_en;

logic [7:0] r_addr;

localparam int unsigned BANK_VALID_COLS = 70;
logic [1:0] ram_full_q;
logic [3:0][BANK_VALID_COLS-1:0] bank_valid_q [0:1];
...
// TODO: what happens when pong/ping is full tooo? how will it transition again? [x]
always_comb begin : fsm_transition
  // write pingpong
  w_sel_n = w_sel_q;
  unique case (w_sel_q)
    RAM0: begin
      if (ldpc_rst) w_sel_n = RAM1;
    end
    RAM1: begin
      if (ldpc_rst) w_sel_n = RAM0;
    end
  endcase
  ...
  unique case (ldpc_state_q)
    INIT: begin
      if (ready_o & init_i) ldpc_state_n = INFO;
...
always_ff @(posedge clk_i or negedge arst_ni) begin : pingpong_state
  for (int i = 0; i < 2; i++) begin
    if (!arst_ni) begin
      ...
      else if ((i == w_sel_q) & ldpc_rst) ram_full_q[i] <= 1; 
    end
  end
end
...
always_comb begin : control_signalling
  ...
  unique case (ldpc_state_q)
    INIT: begin
      if (ready_o & init_i) latch_config = 1; 
    end
    INFO: begin
    end
    PAR_CORE: begin
    ...
    PAR_ADD: begin
      if (last_block_i & add_parity_valid_i) ldpc_rst = 1;
    end
  endcase
...
always_comb begin : w_addr_sel
  unique case (1'(w_sel_q))
    1'b0 : w_addr = 8'(info_col_cnt_q) + 8'd0;
    1'b1 : w_addr = 8'(info_col_cnt_q) + 8'd128;
  endcase
end
...
always_comb for (int j = 0; j < 4; j++)
    w_bank_valid_addr[j] = COL_WIDTH'(w_bank_addr[j]);
...
always_ff @(posedge clk_i or negedge arst_ni) begin : bank_validity_keeper
  if (!arst_ni) begin
    ...
    if (pingpong_rst) begin
      bank_valid_q[r_sel_q] <= '0;
      bank_valid_o <= '0;
    end else if ({1'b0, r_addr_i} < (COL_WIDTH+1)'(BANK_VALID_COLS)) begin
      for (int j = 0; j < 4; j++) begin
        bank_valid_o[j] <= bank_valid_q[r_sel_q][j][r_addr_i];
      end
    end else begin
      bank_valid_o <= '0;
    end
    ...
    for (int j = 0; j < 4; j++) begin
      if (w_en[j]
          && ({1'b0, w_bank_valid_addr[j]} < (COL_WIDTH+1)'(BANK_VALID_COLS))
          && !(pingpong_rst && (w_sel_q == r_sel_q))) begin
        bank_valid_q[w_sel_q][j][w_bank_valid_addr[j]] <= 1'b1;
      end
    end
  end
end
